...
const SingleTicket = ({ arrayOfLottoNumbers, lottoNumbers, isNumberGeneratingFinished }) => {
    ...
    const renderLottoNumbers = () => arrayOfLottoNumbers.map((n) => <div className='my_ticket_number'
        key={n} > {n} </div>)

    return (<div className={checkNumbers() ? 'my_ticket_list_green' : 'my_ticket_list'} >
        {renderLottoNumbers()}
        <span className={messageCondition ? (checkNumbers() ? 'span_message_winner' : 'span_message_loser') : 'span_message'} > {checkNumbers() ? 'Dobitni tiket :) ' : 'Gubitni tiket :('} </span>
